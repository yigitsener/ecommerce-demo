...
def read_yaml(file_path = 'env_variables.yaml'):
    with open(file_path, 'r') as file:
        try:
            ...
            return None
yaml_data = read_yaml()
env_variables = yaml_data.get('env_variables', {})

# The port to run the application on
PORT = int(os.environ.get('PORT', 8080))
# The container ID of your Web GTM container
GTM_WEB_CONTAINER_ID = env_variables.get('GTM_WEB_CONTAINER_ID')
# Flask secret key
SECRET_KEY = env_variables.get('SECRET_KEY')
...
def inject_user_list():
    return dict(user_list=user_data.USER_LIST)
